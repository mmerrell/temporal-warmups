...
public class SupportTriageWorkflowImpl implements SupportTriageWorkflow {
    // Auto-approve for parallel execution demo (skip signal wait)
    // Set to false to require manual approval via signal
    private static final boolean AUTO_APPROVE_FOR_DEMO = false;
    ...
    public TriageResult triageTicket(String ticketId, String ticketText) {
        ...
        try {
            ...
            // Step 3: Human in the loop (or auto-approve for demo)
            if (AUTO_APPROVE_FOR_DEMO) {
                // Auto-approve for parallel execution demo
                Workflow.getLogger(SupportTriageWorkflowImpl.class)
                        .info("Auto-approving ticket for parallel demo (AUTO_APPROVE_FOR_DEMO=true)");
                approved = true;
                approvalReceived = true;
            } else {
                // Original signal wait logic
                Workflow.getLogger(SupportTriageWorkflowImpl.class)
            ...
            // Step 4: Create CRM case (simulated) - use Workflow.currentTimeMillis() for determinism
